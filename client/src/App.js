...
function App() {
  ...
  const handleLoaderReport = () => {
    ...
    loaderReportArr.forEach((report, i) => {
      ...
      }, i * 4000); // Stagger the messages by 2 seconds each
    });
  ...
  useEffect(() => {
    ...
    if (isDark === "true") {
      body.setAttribute("id", "dark-body"); // Add the dark mode id
    } else {
      body.removeAttribute("id"); // Remove the id when not in dark mode
    }
